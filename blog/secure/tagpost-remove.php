<?php
	...
	class PostRemoveHandler {
		...
		protected function isOnlyNumbers($string) {
			return (preg_match("/[^0-9\1]/",$string)===0);
		}
		...
		public function Remove($get){
			...
			if (!$this->isOnlyNumbers($postID)) {						
				$this->message = new Message("error","Invalid ID for Post");
				return -1;
			}
			$status = $this->sqlset->removeTagFrom($postID,$id);
